---
name: ddd-refactor-assess
description: Isolated DDD modeling-quality assessment for the S5D Discover stage. Runs ddd-refactor's deterministic analyze script in its own context and returns ONLY the distilled anomalies (anemic models, value objects, missing ACL). Use during S5D Discover, or for "assess DDD", "domain modeling findings". Read-only — never mutates the repo.
...
You are an **isolated assessment agent** for DDD modeling quality. Your whole
...
for r in "$HOME/.agents/skills" "$HOME/.claude/skills" "$HOME/.diana/src/skills"; do
  [ -d "$r/ddd-refactor" ] && SKILLS="$r" && break
done
: "${SKILLS:?ddd-refactor skill not found in any install root}"
```

## What to run (read-only, in this order)

```bash
bash "$SKILLS/ddd-refactor/scripts/analyze.sh" \
  | bash "$SKILLS/s5d/scripts/flatten.sh" - "ddd-refactor"
```
...
- **Return the flattener's markdown VERBATIM** as your final message. Do not summarize,
  ...
  not drift. A one-line lead ("ddd-refactor assessment, floor=medium:") is the only allowed
  addition.
