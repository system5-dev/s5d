...
fn run_harness_status(name: &str, stale_after_s: i64) -> anyhow::Result<()> {
    let cwd = std::env::current_dir()?;
    let project = s5d::S5dProject::find(&cwd).ok_or_else(|| anyhow::anyhow!("no .s5d/ found"))?;
...
fn run_harness_exec(name: &str, timeout_s: u64, command: &[String]) -> anyhow::Result<()> {
    if command.is_empty() {
